...
public class P100_ConvertRomanLiteral {
  ...
  */
	 ...
	 * Create variable sum and add the last value to the sum
	 * Iterate the input from last item -1
	 *  if current element is not in map, return exception
	 * 	if current index is < index i - 1, subtract the current index value
	 *  else add the current index value to sum
	 *  return sum
	 *  Time : O(m+n) => O(n)
	 *  Space : O(m) 
	 */
	...
	private int convertRomanLiteral(String input) {
		...
		int sum = romanHash.get(input.charAt(input.length()-1));
